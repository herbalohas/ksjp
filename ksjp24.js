...
const defaultConfigString = "fc=0,bc=white,bcss=#fa3,lf=0,nf=0,ls=36,ns=26";
...
const noteFontFamily = ['font-family:monospace;','font-family:Lucida Console;'];
...
const lyricsBegin = '<div class="ly">';
const lyricsSpace = '<span class="sp">&nbsp;</span>';
const lyricsEnd = '</div>';

const freetextBegin = '<div class="freetext">';
...
function renderOutputDocument() {
  fineTuneLyricsOffset();  //set fine-tune before building contents HTML
...
function fineTuneLyricsOffset() {
  var s = Number(lyricsSize);
  lyricsPositionOffset = 26 - (s-32)/2;
}
    ...
    switch (c) {
      ...
      default:  //lyrics characters
        if (iLyricsChar < tdCenterPosition.length) {
          s += '<span class="ly" style="left:'+ (tdCenterPosition[iLyricsChar]+ lyricsPositionOffset)+'px;">' + c + '</span>';  //lyrics position fine tuning here

          iLyricsChar ++;
        } else {
          s += '<span class="ly" style="position:absolute;right:0">' + c + '</span>';
        }
      break;
    }//switch c
  break;//case "lyrics"
...
if (jpContents === undefined) {
  //default demo
  configString = "fc=0,bc=white,bcss=#fff,lf=0,nf=0,ls=20pt,ns=0";
}
else {
